...
const handleShow2 = (plan) => {
  setUpname(plan.name);
  setUplan(plan.id); // ya plan.name — jo aapko show karna ho
  setUpprice(plan.price);
  setUpduration(plan.duration);
  setUpdailyIncome(plan.dailyIncome);
  setUptotalIncome(plan.totalIncome);
  setUprequiredReferrals(plan.requiredReferrals);
  setShow2(true)
};
...
<button className='btn btn-primary w-25 mb-4 py-2'  onClick={handleShow}>Add Plan</button>
      ...
      <button className="start-btn"  onClick={()=>{handleShow2(item  )}}> update plan </button>
    </div>
